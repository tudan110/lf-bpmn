...
class UserTaskView extends RectNode {
  // 重写父类的方法
  getLabelShape(Executionstatus = 0) {
  ...
  getShape() {
    ...
    if (store.state.logicFlow.runState && store.state.logicFlow.runState.length > 0) {
      Executionstatus = executionstatus(store.state.logicFlow.runState, model.id)
    }
    ...
    return h('g', {}, [
      ...
      this.getLabelShape(Executionstatus)
    ])
  }
}
